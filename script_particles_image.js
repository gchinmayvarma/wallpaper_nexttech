...
function particle_image_make() {
  ...
  particle.src = `stuff/${
    image_paths[Math.floor(random(0, image_paths.length))]
  }`;
  particle.custom = {};
...
function particles_image_work() {
  for (let i = 0; i < particle_count_image; i++) {
    ...
    if (particles_image[i].custom.life > particles_image[i].custom.lifeout) {
      ...
      if (
        particles_image[i].custom.life > 2000 ||
        particles_image[i].custom.s < 0.01
      ) {
        particle_count_image--;
    ...
    particles_image[
      i
    ].style.transform = `translate(-50%, -50%) rotate(${particles_image[i].custom.theta}rad) scale(${particles_image[i].custom.s})`;
  }
}
